...
class TvSaleUpload extends StatefulWidget {
  const TvSaleUpload({super.key});

  @override
  State<TvSaleUpload> createState() => _TvSaleUploadState();
}

class _TvSaleUploadState extends State<TvSaleUpload> {
  ...
  final List<Map<String, dynamic>> _giftOptions = [
    ...
    {'name': 'TV Wall Mount', 'icon': Icons.tv},
    {'name': 'HDMI Cable', 'icon': Icons.cable},
    {'name': 'Set Top Box', 'icon': Icons.settings_input_antenna},
    {'name': 'Other', 'icon': Icons.add_circle_outline},
  ];

  // Bill numbers list - only TV bills
  ...
  final Color _tvColor = const Color(0xFF6366F1); // Indigo for TV
  ...
  Future<void> _loadBillNumbers() async {
    try {
      ...
      if (billNumbers.isEmpty) {
        ...
        debugPrint('Loaded ${billNumbers.length} TV bills successfully');
      }
    } catch (e) {
      debugPrint('Error loading TV bills: $e');
  ...
  Future<void> _autofillFromBill(String? billNumber) async {
    ...
    try {
      debugPrint('=== TV BILL DATA STRUCTURE ===');
      debugPrint('All keys in bill: ${billData.keys.join(', ')}');

      debugPrint('type: ${billData['type']}');
      debugPrint('modelName: ${billData['modelName']}');
      debugPrint('serialNumber: ${billData['serialNumber']}');
      ...
      debugPrint('financeType: ${billData['financeType']}');
      ...
      } else if (originalTvData != null &&
          originalTvData['modelPrice'] != null) {
      ...
      String autofillMessage = '✓ Data autofilled from TV bill $billNumber';
      List<String> missingFields = [];

      if (brand.isEmpty) missingFields.add('Brand');
      if (modelName.isEmpty) missingFields.add('Model Name');
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  isEmiMode ? 'TV EMI Sale Uploaded!' : 'TV Sale Uploaded!',
                  style: TextStyle(
                    fontSize: 18,
  ...
  Future<void> _showShareOptionsDialog() async {
    ...
    final message = _generateTvShareMessage();
        ...
        return Dialog(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  isEmiMode ? 'Share TV EMI Details' : 'Share TV Sale Details',
                  style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                    color: _primaryColor,
  ...
  void _uploadTvSale() async {
    ...
    try {
      ...
      final salesData = {
        ...
        'serialNumber': _imeiController.text,
        ...
        'gifts': _selectedGifts.where((g) => g.isNotEmpty).toList(),
      ...
      await _firestore.collection('tvSales').add(salesData);

      setState(() {
        _lastSaleData = {...salesData, 'customerPhone': customerPhone};
      });

      _showMessage('✓ TV sale uploaded successfully!', isError: false);
  ...
  String _generateTvShareMessage() {
    if (_lastSaleData == null) return '';
    ...
    final serialNumber = sale['serialNumber']?.toString() ?? '';
    ...
    final shopWhatsApp = _shopWhatsAppNumber ?? '9072430483';
    ...
    if (isEmiMode) {
      buffer.writeln('✨ *TV EMI DETAILS* ✨');
    } else if (isReadyCashMode) {
      buffer.writeln('✨ *TV SALE DETAILS* ✨');
    }

    buffer.writeln();
    buffer.writeln(' Shop : $shopName');
    buffer.writeln(' Brand : ${brand.toUpperCase()}');
    buffer.writeln(' Model : $model');
    if (serialNumber.isNotEmpty) {
      buffer.writeln(' Serial No : $serialNumber');
    }
    ...
    buffer.writeln();
    buffer.writeln('📺 TV SALES - SERVICE - EXCHANGE');
    buffer.writeln();
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  Text(
                    'TV Model: ${_productModelController.text}',
                    style: const TextStyle(fontSize: 13),
                  ),
                  if (_imeiController.text.isNotEmpty) ...[
                    const SizedBox(height: 6),
                    Text(
                      'Serial No: ${_imeiController.text}',
                      style: const TextStyle(fontSize: 13),
                    ),
                  ],
                  const SizedBox(height: 6),
  ...
  // Bill number field - ONLY TV BILLS
      ...
      children: [
        Row(
          children: [
            ...
            const Spacer(),
            Tooltip(
              message: 'Refresh TV bill list',
              child: GestureDetector(
                onTap: () {
                  _loadBillNumbers();
                  _showMessage('Refreshing TV bills...', isError: false);
                },
                child: Container(
                  padding: const EdgeInsets.all(4),
                  decoration: BoxDecoration(
                    color: _tvColor.withOpacity(0.1),
                    borderRadius: BorderRadius.circular(4),
                  ),
                  child: Icon(Icons.refresh, size: 16, color: _tvColor),
                ),
              ),
            ),
          ],
        ...
        if (!_withoutBillNumber) ...[
          Column(
            ...
            children: [
              Container(
                decoration: BoxDecoration(
                  border: Border.all(color: _tvColor.withOpacity(0.3)),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Column(
                  children: [
                    Padding(
                      ...
                      child: Row(
                        children: [
                          Icon(Icons.search, color: _tvColor, size: 18),
                          const SizedBox(width: 6),
                          Expanded(
                            child: TextField(
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 6),
                            Text(
                              'No TV bills match "${_billSearchController.text}"',
                              style: TextStyle(
                                fontSize: 11,
                                color: _errorColor,
                              ),
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () async {
                          if (_selectedBillNumber != null) {
                            _showMessage(
                              'Re-autofilling data from TV bill...',
                              isError: false,
                            );
                            _clearFormData();
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                'No TV bills found for your shop',
                                style: TextStyle(
                                  fontSize: 11,
                                  color: _warningColor,
                                  fontWeight: FontWeight.w600,
                              ...
                              Text(
                                'Create TV bills first or use "Without Bill Number" option',
                                style: TextStyle(
                                  fontSize: 10,
                                  color: _secondaryColor,
                                ),
          ...
          fieldViewBuilder:
              (
                ...
                return TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Type or search TV brand...',
                    hintStyle: const TextStyle(fontSize: 12),
                    prefixIcon: Icon(Icons.tv, color: _tvColor, size: 18),
                    ...
                    focusedBorder: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(8),
                      borderSide: BorderSide(color: _tvColor, width: 1.5),
                    ),
                    contentPadding: const EdgeInsets.symmetric(
                      horizontal: 10,
                      vertical: 10,
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          ...
                          return ListTile(
                            ...
                            leading: Icon(Icons.tv, size: 16, color: _tvColor),
                            dense: true,
                          );
                        },
                      ),
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.check_circle, size: 12, color: _tvColor),
                  const SizedBox(width: 4),
                  Text(
                    'Selected: ${_selectedBrand!}',
                    style: TextStyle(fontSize: 11, color: _tvColor),
                  ),
                ],
              ),
            ),
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(color: _tvColor, shape: BoxShape.circle),
            child: Icon(Icons.tv, color: Colors.white, size: 12),
          ),
          const SizedBox(width: 6),
          Expanded(
            child: Column(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: (_isLoading || _shopId == null)
            ? null
            : LinearGradient(
                colors: [_tvColor, _tvColor.withOpacity(0.7)],
      ...
      child: ElevatedButton(
        onPressed: (_isLoading || _shopId == null) ? null : _uploadTvSale,
        style: ElevatedButton.styleFrom(
        ...
        child: _isLoading
            ...
            : Text(
                _shopId == null ? 'Waiting for Shop Info' : 'Upload TV Sale',
                style: TextStyle(
                  fontSize: 13,
                  fontWeight: FontWeight.bold,
                  color: Colors.white,
  ...
  Widget build(BuildContext context) {
    return MediaQuery(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: const Text('TV Sale Upload', style: TextStyle(fontSize: 16)),
          backgroundColor: _tvColor,
          foregroundColor: Colors.white,
          elevation: 0,
            ...
            children: [
              ...
              Card(
                ...
                child: Padding(
                  padding: const EdgeInsets.all(12),
                  child: Column(
                    children: [const SizedBox(height: 16), _buildTvSaleForm()],
                  ),
                ),
              ),
              const SizedBox(height: 16),
