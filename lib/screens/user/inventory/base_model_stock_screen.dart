...
class _BaseModelStockScreenState extends State<BaseModelStockScreen>
  ...
  Future<void> _loadExistingProducts() async {
    try {
      ...
      final snapshot = await _firestore.collection('baseModels').get();
  ...
  Future<void> _saveNewProduct() async {
    ...
    try {
      ...
      await _firestore.collection('baseModels').add(newProduct);
  ...
  Future<void> _saveStock() async {
    ...
    try {
        ...
        if (_showPriceChangeOption && _priceChangeController.text.isNotEmpty) {
          ...
          if (newPrice != null && newPrice > 0) {
            ...
            if (productId != null && productId != 'temp') {
              await _firestore.collection('baseModels').doc(productId).update({
                'price': productPrice,
      ...
      for (int i = 0; i < _imeiNumbers.length; i++) {
        ...
        final docRef = _firestore.collection('baseModelStock').doc();
      ...
      _showSuccess('Successfully added $savedCount base model(s) to stock!');
  ...
  Future<void> _transferToShop(
    ...
    try {
      ...
      _showSuccess('Base model transferred to $newShopName successfully!');
    } catch (e) {
      _showError('Failed to transfer base model: $e');
    } finally {
      if (mounted) {
        setState(() => _isLoading = false);
      }
  ...
  Future<void> _returnModel(
    ...
    try {
      ...
      await _firestore.collection('baseModelReturns').add(returnData);

      await _firestore.collection('baseModelStock').doc(modelId).delete();

      setState(() {
        _selectedModelForAction = null;
      });

      _showSuccess('Base model returned successfully!');
    } catch (e) {
      _showError('Failed to return base model: $e');
    } finally {
      if (mounted) {
        setState(() => _isLoading = false);
      }
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 4),
                            Text(
                              'This base model was previously sold',
                              style: TextStyle(
                                fontSize: 11,
  ...
  Widget _buildActionModal() {
    ...
    final imei = model['imei'] as String? ?? 'N/A';
          ...
          child: Column(
            ...
            children: [
              ...
              if (_selectedAction == 'sell') ...[
                const Text(
                  'Are you sure you want to mark this base model as sold?',
                ...
                const Text(
                  'Are you sure you want to return this base model?',
                  style: TextStyle(fontSize: 12, color: Colors.orange),
                ),
                const SizedBox(height: 8),
                const Text(
                  'This will remove the base model from available stock and create a return record.',
              ...
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      'Error loading returned base models: ${snapshot.error}',
                      textAlign: TextAlign.center,
            ...
            children: [
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Text(
                      'Returned Models: ${filteredReturns.length}',
