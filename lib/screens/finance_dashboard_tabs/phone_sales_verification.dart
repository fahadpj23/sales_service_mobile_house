// lib/screens/admin/reports/phone_sales_verification_tab.dart
import 'package:flutter/material.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:intl/intl.dart';

class PhoneSalesVerificationTab extends StatefulWidget {
  ...
  @override
  State<PhoneSalesVerificationTab> createState() =>
      _PhoneSalesVerificationTabState();
}

class _PhoneSalesVerificationTabState extends State<PhoneSalesVerificationTab> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        _buildVerificationSummary('Phone Sales', data, widget.allData),
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text(
                        'No phone sales found for verification',
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return Padding(
                      ...
                      child: _buildPhoneSaleCard(data[index], context),
                    );
                  },
                ),
        ),
  ...
  Future<void> _verifyDownPaymentRow(Map<String, dynamic> sale) async {
    ...
    try {
      ...
      await _firestore.collection('phoneSales').doc(docId).update(updates);
  ...
  Future<void> _maybeFinalizePayment(Map<String, dynamic> sale) async {
    ...
    if (downOk && disbOk && sale['paymentVerified'] != true) {
      try {
        await _firestore.collection('phoneSales').doc(docId).update({
  ...
  Widget _buildPhoneSaleCard(Map<String, dynamic> sale, BuildContext context) {
    ...
    String customerName = sale['customerName']?.toString() ?? '';
    String customerPhone = sale['customerPhone']?.toString() ?? '';
    String brand = sale['brand']?.toString() ?? '';
    String productModel = sale['productModel']?.toString() ?? '';
    String description = '$brand $productModel'.trim();
    dynamic date =
        sale['saleDate'] ??
        sale['createdAt'] ??
        sale['date'] ??
          ...
          children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: widget.getPaymentModeColor(purchaseMode),
                          borderRadius: BorderRadius.circular(4),
                          border: Border.all(
                            color: widget.getPaymentModeBorderColor(
                              purchaseMode,
                            ),
                            width: 1,
                          ),
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            fontSize: 10,
                            fontWeight: FontWeight.w500,
                            color: widget.getPaymentModeTextColor(purchaseMode),
                          ),
                        ),
                      ),
                    ],
            ...
            if (customerPhone.isNotEmpty) ...[
              ...
              Wrap(
                spacing: 6,
                runSpacing: 4,
                children: [_buildTag('📞 $customerPhone', Colors.blue)],
              ),
            ],
  ...
  // ── Non-EMI verify banner (opens parent dialog) ──
